...
interface ItemGroupContextType {
    ...
    updateGroup: (
        id: string,
        data: Partial<ItemGroupModel>,
    ) => Promise<boolean>;
    deleteGroup: (id: string) => Promise<boolean>;
}

const { Context: ItemGroupContext, useGenericContext } =
    createGenericContext<ItemGroupModel>();
...
export function useItemGroup(): ItemGroupContextType {
    const ctx = useGenericContext(
        "useItemGroup",
    ) as unknown as GenericContextType<ItemGroupModel>;
    return {
        groups: ctx.list,
        total: ctx.total,
