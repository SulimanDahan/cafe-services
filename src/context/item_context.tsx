...
interface ItemContextType {
 ...
 updateItem: (id: string, data: Partial<ItemModel>) => Promise<boolean>;
 deleteItem: (id: string) => Promise<boolean>;
}

const { Context: ItemContext, useGenericContext } = createGenericContext<ItemModel>();
...
export function ItemProvider({ children }: { children: ReactNode }) {
 ...
 [items, total, totalPages, isItemsLoading, fetchAllItems, addItem, updateItem, deleteItem]
 );
...
export function useItem(): ItemContextType {
 const ctx = useGenericContext("useItem") as unknown as GenericContextType<ItemModel>;
 return {
 items: ctx.list,
 total: ctx.total,
