...
interface OrderContextType {
    ...
    fetchAllOrders: (queryParams?: Record<string, string>) => Promise<void>;
    addOrder: (data: Partial<OrderModel>) => Promise<boolean>;
    updateOrder: (id: string, data: Partial<OrderModel>) => Promise<boolean>;
    deleteOrder: (id: string) => Promise<boolean>;
}

const { Context: OrderContext, useGenericContext } =
    createGenericContext<OrderModel>();
    ...
    useEffect(() => {
        ...
        const onOrderDeleted = (event: MessageEvent) => {
            try {
                ...
                setOrders((prev) =>
                    prev.filter((o) => o.id !== deletedOrderId),
                );
            } catch (e) {
...
export function useOrder(): OrderContextType {
    const ctx = useGenericContext(
        "useOrder",
    ) as unknown as GenericContextType<OrderModel>;
    return {
        orders: ctx.list,
