...
export async function GET(request: Request) {
    ...
    // Allow public access ONLY for fetching the dropdown list (used by homepage booking modal)
    if (!fetchAll && !(await requireAuth())) {
...
export async function POST(request: Request) {
    ...
    try {
        ...
        return NextResponse.json(
            { error: "apiMessages.error.createRoomFailed" },
            { status: 500 },
        );
    }
}
