...
export async function PUT(request: Request, { params }: Params) {
 ...
 if (!currentRes) return NextResponse.json({ error: "apiMessages.error.reservationNotFound" }, { status: 404 });

 const isAccepted = data.accepted !== undefined ? data.accepted : currentRes.accepted;
 const isCompleted = data.completed !== undefined ? data.completed : currentRes.completed;
