...
export async function GET(request: Request) {
	...
	try {
		...
		return NextResponse.json(
			{ error: "apiMessages.error.fetchOrdersFailed" },
			{ status: 500 },
		);
	}
}
...
export async function POST(request: Request) {
	...
	try {
		...
		const adminOrderSettings = await getSystemSettings();
		const autoAccept = adminOrderSettings?.auto_accept_orders ?? false;
		...
		return NextResponse.json(
			{ error: "apiMessages.error.createOrderFailed" },
			{ status: 500 },
		);
	}
}
