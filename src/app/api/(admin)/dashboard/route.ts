...
export async function GET(req: NextRequest) {
	...
	try {
		...
		const whereOrder: Prisma.OrderWhereInput = {
			accepted: true, // Only count approved orders for revenue and units
		};
		...
		if (range !== "all") {
			...
			whereReservation.created_at = { gte: start, lte: end };
		...
		const totalRevenue = ordersList.reduce(
			(
				sum: number,
				o: { item_price: Prisma.Decimal; quantity: number },
			) => sum + Number(o.item_price) * o.quantity,
			0,
		);
		const totalOrderedUnits = ordersList.reduce(
			(
				sum: number,
				o: { item_price: Prisma.Decimal; quantity: number },
			) => sum + o.quantity,
