...
export default function CustomerOrderPage() {
		...
		if (isScannerOpen) {
			const startCamera = async () => {
				try {
					...
					console.warn("Camera access failed or user rejected permission. Fallback active.", err);
					setScanStep("idle");
					setScanErrorMsg(
						isRtl
			...
			// Defer camera initialization to allow modal UI to render instantly on mobile devices
	...
	const handleSimulateScan = (roomId: string, tableName: string) => {
		...
		// Simulate 1.5 seconds decoding and security checks
		setTimeout(() => {
