...
import { PlusIcon, SunIcon, UndoCircleIcon, CheckIcon, TrashIcon, PrintIcon } from "@/components/icons";
...
export default function AdminRoomsPage() {
	...
	const { rooms, total, totalPages, isRoomsLoading, fetchAllRooms, addRoom, updateRoom, deleteRoom } = useRoom();

	const columns: TableColumn[] = [
		{ key: "identifier", label: t("reservations.colRoomIdentifier") },
		{ key: "name", label: t("reservations.colRoomTableName") },
		{ key: "qr_code", label: t("reservations.colQrToken") },
		{ key: "status", label: t("reservations.colOperatingStatus"), align: "center" },
		{ key: "qr_actions", label: t("reservations.colQrActions"), align: "center" },
		{ key: "actions", label: t("reservations.colOperations"), align: "center" },
	];
	...
	const handleAddRoomSubmit = async (data: RoomInput) => {
		...
		const isTokenDuplicate = rooms.some((r) => r.qr_code.toLowerCase() === lowerToken);
		if (isTokenDuplicate) {
		...
		<div className="space-y-6">
			...
			<style jsx global>{`
				@media print {
					...
					#printable-sticker {
						...
						border: 2px dashed #000000 !important; border-radius: 16px !important;
			...
			<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
				<div className="rounded-[28px] border border-white/10 bg-[#131522] p-6 flex flex-col justify-between gap-3 shadow-md relative overflow-hidden group hover:border-amber-500/20 transition-colors">
					...
					<span className="text-xs text-zinc-400 font-bold uppercase tracking-wide">{t("reservations.statTotalTables")}</span>
					<span className="text-4xl font-black text-white">{rooms.length}</span>
				</div>
				<div className="rounded-[28px] border border-white/10 bg-[#131522] p-6 flex flex-col justify-between gap-3 shadow-md relative overflow-hidden group hover:border-green-500/20 transition-colors">
					...
					<span className="text-xs text-zinc-400 font-bold uppercase tracking-wide">{t("reservations.statActiveTables")}</span>
					<span className="text-4xl font-black text-green-400">{rooms.filter((r) => !r.is_disable).length}</span>
				</div>
				<div className="rounded-[28px] border border-white/10 bg-[#131522] p-6 flex flex-col justify-between gap-3 shadow-md relative overflow-hidden group hover:border-red-500/20 transition-colors">
					...
					<span className="text-xs text-zinc-400 font-bold uppercase tracking-wide">{t("reservations.statOutOfService")}</span>
					<span className="text-4xl font-black text-red-400">{rooms.filter((r) => r.is_disable).length}</span>
				</div>
			</div>
			...
			<div className="rounded-[28px] border border-white/10 bg-[#131522] p-6 shadow-md space-y-6">
				<div className="flex flex-col sm:flex-row justify-between items-center gap-4">
					<SearchInput value={searchQuery} onChange={(v) => { setSearchQuery(v); setCurrentPage(1); }} placeholder={t("reservations.roomsSearch")} />
				</div>
				...
				>
					{rooms.map((room) => (
						<tr key={room.id} className="group hover:bg-[#1a1c2c]/40 transition-colors duration-200">
							<td className={`py-4 px-4 font-black text-xs text-zinc-500 whitespace-nowrap ${isRtl ? "text-right" : "text-left"}`}>{room.id.slice(0, 8)}…</td>
							<td className={`py-4 px-4 font-bold text-white group-hover:text-amber-300 transition-colors whitespace-nowrap ${isRtl ? "text-right" : "text-left"}`}>{room.name}</td>
							<td className={`py-4 px-4 font-black text-xs text-amber-400 whitespace-nowrap ${isRtl ? "text-right" : "text-left"}`}>
								<span className="px-2 py-1 rounded bg-[#07080a]/80 border border-white/5 font-mono uppercase tracking-wide">{room.qr_code}</span>
							</td>
							<td className="py-4 px-4 text-center whitespace-nowrap">
								<Badge
									variant={!room.is_disable ? "success" : "error"}
									pulse
								>
									{!room.is_disable ? t("reservations.statusActiveAvailable") : t("reservations.statusOutOfService")}
								</Badge>
							</td>
							<td className="py-4 px-4 text-center whitespace-nowrap">
								<button
							...
							<td className="py-4 px-4 text-center whitespace-nowrap">
								<div className="flex items-center justify-center gap-2">
									<button
										...
										className={`p-1.5 rounded-lg border transition-all duration-200 cursor-pointer ${!room.is_disable
										...
										title={!room.is_disable ? t("reservations.actionDeactivate") : t("reservations.actionActivate")}
									>
										{!room.is_disable ? (
			...
			>
				<form onSubmit={handleSubmit(handleAddRoomSubmit)} className="space-y-4">
					<div className="space-y-1.5">
						<label htmlFor="rName" className="text-xs font-bold text-zinc-400 block">{t("reservations.formRoomName")}</label>
						<input
							id="rName"
							type="text"
							{...register("name")}
						...
						/>
						{errors.name && (
							<p className="text-[10px] text-red-400 font-medium mt-1">{errors.name.message}</p>
						)}
					</div>

					<div className="space-y-1.5">
						<label htmlFor="rToken" className="text-xs font-bold text-zinc-400 block">{t("reservations.formQrToken")}</label>
						<input
							id="rToken"
							type="text"
							{...register("qr_code")}
						...
						/>
						{errors.qr_code && (
							<p className="text-[10px] text-red-400 font-medium mt-1">{errors.qr_code.message}</p>
						)}
						<p className="text-[10px] text-zinc-500 leading-normal font-medium mt-1">{t("reservations.formQrTokenDesc")}</p>
					</div>
			...
			{isPreviewOpen && selectedRoom && (
				...
				>
					<div className="space-y-6">
						<p className="text-xs text-zinc-400 -mt-2 text-center">{t("reservations.stickerSub")}</p>

						<div id="printable-sticker" className="mx-auto w-72 rounded-3xl border border-amber-500/30 bg-[#07080a] p-6 text-center space-y-5 shadow-xl relative overflow-hidden flex flex-col items-center justify-center">
							<div className="flex items-center justify-center gap-2">
								<div className="h-7 w-7 rounded-lg bg-amber-500/20 border border-amber-500/40 text-amber-400 flex items-center justify-center font-black text-sm">☕</div>
								<div className="text-left font-black tracking-wide leading-none text-white print:text-black">
									<p className="text-[10px] print:text-black">{t("reservations.stickerCafe")}</p>
									<p className="text-[7px] text-amber-400/90 tracking-widest uppercase mt-0.5">SERVICES</p>
								</div>
							</div>

							<div className="space-y-1.5">
								<h3 className="text-base font-black text-white print:text-black leading-tight">{selectedRoom.name}</h3>
								<span className="inline-flex items-center px-2 py-0.5 rounded bg-[#131522] print:bg-zinc-100 border border-white/5 print:border-black/10 text-[8px] font-mono text-zinc-400 print:text-zinc-600 uppercase">
									{t("reservations.stickerId")} {selectedRoom.qr_code}
								</span>
							</div>
							...
							<div className="relative h-44 w-44 bg-white rounded-2xl p-3 shadow-inner flex items-center justify-center border border-white/10 print:border-black/25">
								{isGeneratingQr ? (
									...
									previewQrUrl && (
										<Image src={previewQrUrl} alt={`${selectedRoom.name} QR Code`} width={176} height={176} className="h-full w-full object-contain" unoptimized />
									)
								)}
							</div>

							<div className="space-y-1">
								<p className="text-[10px] font-black text-amber-400 print:text-black uppercase tracking-wide">{t("reservations.stickerScan")}</p>
								<p className="text-[8px] text-zinc-500 print:text-zinc-600 leading-normal max-w-50 mx-auto">{t("reservations.stickerScanDesc")}</p>
							</div>
						</div>
