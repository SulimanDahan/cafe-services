...
// import { QrCodeIcon, QrCodeNeonIcon } from "@/components/icons/qrcode_icon";
...
//                     // <main className="flex-1 max-w-7xl w-full mx-auto bg-red-500 px-4 sm:px-6 md:px-8 py-10 flex flex-col gap-8">
...
import PWAInstallBanner from "@/components/PWAInstallBanner";
import { PrimaryButton } from "@/components/button/primary_button";
import { QrCodeNeonIcon } from "@/components/icons/qrcode_icon";
import LockIcon from "@/components/icons/lock_icon";
...
export default function CustomerOrderPage() {
    ...
    useEffect(() => {
        ...
        if (link) { link.href = "/customer-manifest.json"; }
        else {
            link = document.createElement("link");
            link.rel = "manifest";
            link.href = "/customer-manifest.json";
    ...
    function handleQrDecoded(qrData: string, activeStream: MediaStream) {
        ...
        if (matchedRoomId) {
            ...
            setScanStep("error");
            setScanErrorMsg(isRtl ? `تمت قراءة كود QR بنجاح: (${qrData}) ولكن الرمز لا يطابق أي طاولة.` : `Scanned successfully: (${qrData}) but no match found.`);
            setTimeout(() => {
                if (isScannerOpen && videoRef.current) {
                    setScanStep("scanning");
                    setScanErrorMsg("");
            ...
            const startCamera = async () => {
                try {
                    if (typeof navigator !== "undefined" && navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
                        ...
                        setScanStep("idle");
                        setScanErrorMsg(isRtl ? "ملاحظة: المتصفح يمنع الكاميرا في الأوضاع غير الآمنة (HTTP). استخدم محاكاة المسح أدناه." : "Note: Insecure context (HTTP) blocks camera access.");
                    }
                } catch {
                    setScanStep("idle");
                    setScanErrorMsg(isRtl ? "ملاحظة: يرجى تفعيل صلاحيات الكاميرا وتشغيل الـ HTTPS. تم تفعيل نظام محاكاة الاستقبال." : "Note: Camera access failed. Fallback prototype active.");
                }
            };
            ...
            if (storedSession) {
                try {
                    ...
                    if (!stillExists) {
                        ...
                        localStorage.removeItem("cafe_active_session");
                        setActionMessage({
                            text: isRtl ? "تم إنهاء الحجز من قبل الاستقبال وتصفية الحساب. شكراً لزيارتك!" : "Reservation ended by reception. Thank you!",
                            isError: false,
                        });
                    } else {
                        setActiveSession(sObj);
    ...
    function handleSimulateScan(roomId: string, tableName: string) {
        ...
        setTimeout(() => {
            ...
            const todayActiveRes = roomReservations.find(
                (r) => r.accepted && (r.datetime.includes(todayAr) || r.datetime.includes(todayEn))
            );
            ...
            if (todayActiveRes) {
                ...
                setScanLoading(false);
                setActionMessage({
                    text: isRtl ? `أهلاً بك عميلنا المميز (${todayActiveRes.client_name})! تم فتح الجلسة لطاولة ${tableName}.` : `Welcome (${todayActiveRes.client_name})! Session unlocked.`,
                });
                ...
                if (anyRes) {
                    if (!anyRes.accepted) {
                        setScanErrorMsg(isRtl ? `عذراً! الحجز باسم (${anyRes.client_name}) لم يتم تأكيده بعد.` : `Reservation pending confirmation.`);
                    } else {
                        setScanErrorMsg(isRtl ? `عذراً! لا يوجد حجز مؤكد لتاريخ اليوم على هذه الطاولة.` : `No confirmed reservation for today.`);
                    }
                } else {
                    setScanErrorMsg(isRtl ? `عذراً! لا توجد أي حجوزات مسجلة على طاولة ${tableName}.` : `No reservations registered.`);
                }
            }
        }, 1200);
    }
    ...
    const handleLogOutSession = () => {
        setActiveSession(null);
        localStorage.removeItem("cafe_active_session");
        setActionMessage({ text: isRtl ? "تم تسجيل الخروج وقفل الجلسة." : "Logged out successfully." });
    };
    ...
    const handlePlaceOrder = (item: MenuItem) => {
        ...
        updateLocalStorageOrders([newOrder, ...orders]);
    ...
    const handleCancelOrder = (orderId: string, itemName: string) => {
        ...
        setActionMessage({ text: isRtl ? `تم إلغاء طلب (${itemName}) بنجاح.` : `Cancelled (${itemName}).`, isError: true });
        window.dispatchEvent(new CustomEvent("orders-updated"));
    };
                                            ...
                                            <div key={item.id} className="rounded-3xl border border-white/10 bg-[#131522] p-5.5 hover:border-amber-500/30 transition-all duration-300 flex flex-col justify-between gap-4 group shadow-md">
                                                <div className="space-y-2">
                                                    <div className="flex justify-between items-start gap-2">
                                                        ...
                                                        <span className="text-xs font-black text-amber-400">{item.price.toLocaleString("en-US")} {t("common.currency")}</span>
                                                    </div>
                                                ...
                                                <div className="pt-2 border-t border-white/5 flex items-center justify-between gap-3">
                                                    ...
                                                    <button onClick={() => handlePlaceOrder(item)} className="flex-1 py-2.5 px-4 rounded-full bg-amber-500 hover:bg-amber-400 text-[#07080a] font-black text-xs transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 active:scale-95 shadow-md flex items-center justify-center gap-1.5 cursor-pointer">
                                                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" /></svg>
