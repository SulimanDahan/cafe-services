...
export default function OrdersAdmin() {
	...
	const [menuItems] = useState<Item[]>([
		{ id: "i1", name_ar: "إسبريسو مزدوج", name_en: "Double Espresso", price: 14000 },
		{ id: "i2", name_ar: "كابتشينو كلاسيك", name_en: "Classic Cappuccino", price: 18000 },
		{ id: "i3", name_ar: "سبانش لاتيه بارد", name_en: "Cold Spanish Latte", price: 22000 },
		{ id: "i4", name_ar: "كرواسون الزبدة المقرمش", name_en: "Crispy Butter Croissant", price: 16000 },
		{ id: "i5", name_ar: "كيكة العسل والزعفران", name_en: "Honey Saffron Cake", price: 28000 },
	]);
	...
	const handleAddOrder = (e: React.FormEvent) => {
		...
		const newOrder: Order = {
			...
			item_name: isRtl ? itemRef.name_ar : itemRef.name_en,
	...
	return (
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
				...
				/>
				<MetricCard
					title={isRtl ? "عدد الطلبات النشطة" : "Active Orders Count"}
					value={`${filteredOrders.length} ${isRtl ? "طلبات" : "Orders"}`}
					icon={
						<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
				...
				/>
				<MetricCard
					title={isRtl ? "إجمالي الكميات المباعة" : "Total Sold Units"}
					value={`${filteredOrders.reduce((sum, o) => sum + o.quantity, 0)} ${isRtl ? "وحدات" : "Units"}`}
					icon={
						<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
			...
			<div className="rounded-[28px] border border-white/10 bg-[#131522] p-6 shadow-md space-y-6">
				<div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
					...
					<SearchInput
						...
						placeholder={isRtl ? "البحث باسم العميل أو الصنف..." : "Search client or menu item..."}
					/>
					<span className="text-xs text-zinc-400 font-bold">
						{isRtl ? "إجمالي الطلبات المعروضة:" : "Total Displayed Orders:"} {filteredOrders.length}
					</span>
				</div>
					...
					<table className="min-w-212.5 w-full border-collapse text-sm">
						<thead>
							<tr className={`border-b border-white/10 text-zinc-400 text-xs font-black ${isRtl ? "text-right" : "text-left"}`}>
								<th className="pb-3 px-4">{t("orders.columnId")}</th>
								<th className="pb-3 px-4">{isRtl ? "رقم حجز العميل" : "Booking #"}</th>
								<th className="pb-3 px-4">{t("orders.columnClient")}</th>
								<th className="pb-3 px-4">{isRtl ? "الصنف المطلـوب" : "Requested Item"}</th>
								<th className="pb-3 px-4">{isRtl ? "سعر الوحدة" : "Unit Price"}</th>
								<th className="pb-3 px-4">{isRtl ? "الكمية" : "Quantity"}</th>
								<th className="pb-3 px-4">{t("orders.columnPrice")}</th>
								<th className="pb-3 px-4">{isRtl ? "ساعة طلب الخدمة" : "Order Date & Time"}</th>
								<th className="pb-3 px-4 text-center">{isRtl ? "الإجراء" : "Action"}</th>
							</tr>
						</thead>
							...
							{filteredOrders.length > 0 ? (
								filteredOrders.map((o) => (
									<tr key={o.id} className="group hover:bg-[#1a1c2c]/40 transition-colors duration-200">
										...
										<td className={`py-4 px-4 font-black text-xs text-amber-500 whitespace-nowrap ${isRtl ? "text-right" : "text-left"}`}>
										...
										<td className={`py-4 px-4 font-bold text-white whitespace-nowrap ${isRtl ? "text-right" : "text-left"}`}>
											{o.item_name}
										</td>
										...
										<td className="py-4 px-4 text-center whitespace-nowrap">
											<div className="flex items-center justify-center">
												<button
													...
													title={isRtl ? "حذف الطلب وتحصيل السعر" : "Delete Order"}
												>
													<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
			...
			{isOpen && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" dir={isRtl ? "rtl" : "ltr"}>
					<div className="max-w-md w-full rounded-3xl border border-white/15 bg-[#131522]/95 backdrop-blur-xl p-6 shadow-2xl relative">
						...
						<h2 className="text-lg font-black text-white mb-4">{isRtl ? "إضافة طلب وصنف جديد لعميل" : "Add Order to Booking"}</h2>

						<form onSubmit={handleAddOrder} className="space-y-4">
							{/* Client Booking Matcher */}
							<div className="space-y-1.5">
								<label htmlFor="resSelect" className="text-xs font-bold text-zinc-400 block">
									{isRtl ? "طاولة العميل صاحب الطلب" : "Assigned Client Table"}
								</label>
								<select
									id="resSelect"
									value={selectedResId}
								...
								>
									{reservations.map((r) => (
										<option key={r.id} value={r.id}>
											{r.client_name} ({isRtl ? "حجز" : "Booking"}: {r.number})
										</option>
									))}
								</select>
							</div>
							...
							{/* Menu Item Picker */}
							<div className="space-y-1.5">
								<label htmlFor="itemSelect" className="text-xs font-bold text-zinc-400 block">
									{isRtl ? "الصنف أو المشروب المطلوب" : "Requested Menu Item"}
								</label>
								<select
									id="itemSelect"
							...
							{/* Quantity Number */}
							<div className="space-y-1.5">
								<label htmlFor="qtyIn" className="text-xs font-bold text-zinc-400 block">
