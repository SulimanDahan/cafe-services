...
export default function ItemsAdmin() {
	...
	const [items, setItems] = useState<Item[]>([
		...
		{
			id: "i6",
			name_ar: "سويت كولد برو",
			name_en: "Sweet Cold Brew",
			price: 24000,
			groupId: "g2",
			is_disable: true,
	...
	const [groups] = useState<ItemGroup[]>([
		...
		{
			id: "g3",
			name_ar: "المعجنات والمخبوزات",
			name_en: "Pastries & Bakery",
		},
		{ id: "g4", name_ar: "الحلويات الفاخرة", name_en: "Premium Desserts" },
		{
			id: "g5",
			name_ar: "ركن القهوة المختصة",
			name_en: "Specialty Coffee Bar",
		},
	]);
	...
	const filteredItems = items.filter((item) => {
		...
		const matchesSearch = targetName
			.toLowerCase()
	...
	const handleToggleDisable = (id: string) => {
		setItems((prev) =>
			prev.map((i) =>
				i.id === id ? { ...i, is_disable: !i.is_disable } : i,
			),
		);
	};
			...
			<div className="rounded-[28px] border border-white/10 bg-[#131522] p-6 shadow-md space-y-6">
				<div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
					...
					<span className="text-xs text-zinc-400 font-bold shrink-0">
						{t("item.totalDisplayedItems")} {filteredItems.length}
					</span>
				</div>
				...
				<div className="overflow-x-auto overflow-y-auto max-h-100 lg:max-h-[calc(100vh-340px)]">
					<table className="min-w-212.5 w-full border-collapse text-sm">
						<thead>
							...
							>
								<th className="pb-3 px-4 whitespace-nowrap">
									{t("item.colNameAr")}
								</th>
								<th className="pb-3 px-4 whitespace-nowrap">
									{t("item.colNameEn")}
								...
								<th className="pb-3 px-4 whitespace-nowrap">
									{t("item.columnCreated")}
								</th>
								<th className="pb-3 px-4 text-center whitespace-nowrap">
									{t("common.status")}
								</th>
								<th className="pb-3 px-4 text-center whitespace-nowrap">
									{t("common.actions")}
								...
								filteredItems.map((item) => {
									...
									return (
										...
										>
											...
											<td
												className={`py-4 px-4 font-semibold text-zinc-300 whitespace-nowrap ${isRtl ? "text-right" : "text-left"}`}
											>
												{item.name_en}
											</td>
											<td
											...
											<td
												className={`py-4 px-4 text-amber-400 font-black text-xs whitespace-nowrap ${isRtl ? "text-right" : "text-left"}`}
											>
