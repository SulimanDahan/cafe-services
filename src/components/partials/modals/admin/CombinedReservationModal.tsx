...
interface CombinedReservationModalProps {
    ...
    onUndoAction: (id: string, action: "accept" | "activate" | "complete") => Promise<void>;
...
export default function CombinedReservationModal({
    ...
    return (
        <AdminModal isOpen={isOpen} onClose={onClose} maxWidth={reservation ? "max-w-6xl" : "max-w-md"}>
            <div className={`flex flex-col gap-6 lg:gap-8 w-full max-h-[75vh] lg:max-h-[85vh] overflow-hidden pr-1 lg:pr-0 transition-all duration-500`}>
                ...
                <div className={`w-full flex-1 flex flex-col lg:grid ${reservation ? 'lg:grid-cols-[35%_minmax(0,1fr)]' : 'lg:grid-cols-1'} gap-6 lg:gap-8 min-h-0`}>
                    ...
                    {/* LEFT COLUMN: DETAILS    */}
                    {/* ======================= */}
                    <div className={`flex flex-col min-h-0 ${reservation ? "w-full overflow-hidden" : "w-full"} ${activeTab !== "details" && reservation ? "hidden lg:flex" : "flex"}`}>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 lg:pr-2 pb-4">
                            <form id="reservation-form" onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5">
                                <InputField
                                    ...
                                    isSelect
                                    {...register("room_id")}
                                    disabled={!reservation || !reservation.activated || reservation.completed}
                        ...
                        <div className="shrink-0 pt-6 mt-4 border-t border-white/5 w-full flex gap-3">
                            <PrimaryButton type="submit" form="reservation-form" className="flex-1 shadow-lg shadow-primary/20">
                                {t("common.save")}
                            </PrimaryButton>
                            {reservation && !reservation.accepted && !reservation.completed && (
                                <PrimaryButton variant="secondary" type="button" onClick={() => onDelete(reservation.id)} className="shrink-0 text-zinc-400 hover:text-red-400 hover:bg-red-400/10 border-transparent hover:border-red-400/20">
                    ...
                    {/* ======================= */}
                    {reservation && (
                        <div className={`flex flex-col min-h-0 overflow-hidden gap-4 ${activeTab === "details" ? "hidden lg:flex" : "flex"}`}>
                            <div className="flex-1 flex flex-col bg-[#07080a]/50 rounded-3xl border border-white/5 p-4 lg:p-6 shadow-[inset_0_0_40px_rgba(0,0,0,0.5)] min-h-0 overflow-hidden">
                                    ...
                                    {rightPanelTab === "orders" && (
                                        <div className="space-y-3 animate-fade-in h-full">
                                            {orders.length === 0 ? (
                                                ...
                                                orders.map((o) => (
                                                    <div key={o.id} className="p-4 rounded-[20px] bg-surface/80 backdrop-blur-md border border-white/5 flex flex-col gap-3 shadow-sm hover:border-white/10 transition-colors">
                                                        {/* Details & Price Row */}
                                                        <div className="flex justify-between items-start gap-4">
                                                            <div className="flex-1 min-w-0">
                                                                <h4 className="font-black text-white text-base truncate">{o.item?.name ?? t("common.unknown")}</h4>
                                                                <div className="text-xs text-zinc-400 font-medium mt-1.5 flex items-center gap-2">
                                                                    <span className="font-black text-white bg-white/10 px-2 py-0.5 rounded-md border border-white/5">{o.quantity}</span>
                                                                    <span>× {Number(o.item_price).toLocaleString("en-US")} {t(`common.${settings.currency_name}`)}</span>
                                                                </div>
                                                                {o.notes && (
                                                                    <p className="text-[11px] text-amber-400 font-bold mt-2 bg-amber-400/10 inline-flex items-center px-2.5 py-1 rounded-lg border border-amber-400/20">
                                                                        {t("orders.notePrefix")}{o.notes}
                                                                    </p>
                                                                )}
                                                            </div>
                                                            <div className="shrink-0 text-right">
                                                                <span className="font-black text-white text-sm bg-black/40 px-3 py-1.5 rounded-xl border border-white/5 inline-flex items-center gap-1.5 shadow-inner">
                                                                    {(Number(o.item_price) * o.quantity).toLocaleString("en-US")} <span className="text-[10px] text-primary">{t(`common.${settings.currency_name}`)}</span>
                                                                </span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="flex items-center justify-between pt-3 mt-1 border-t border-white/5">
                                                            {o.accepted ? (
                                                                <>
                                                                    <span className="px-3 py-1.5 rounded-xl text-[11px] font-black bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center gap-2">
                                                                        <CheckIcon className="w-4 h-4" />
                                                                        {t("orders.statusApproved")}
                                                                    </span>
                                                                    {!reservation.completed && (
                                                                        <button onClick={() => onUnapproveOrder(o.id)} className="p-2 rounded-xl text-zinc-500 hover:text-amber-400 hover:bg-amber-400/10 border border-transparent hover:border-amber-400/20 transition-all cursor-pointer" title={t("common.undo")}>
                                                                            <UndoCircleIcon className="w-5 h-5" />
                                                                        </button>
                                                                    )}
                                                                </>
                                                            ) : (
                                                                !reservation.completed ? (
                                                                    <>
                                                                        <button onClick={() => onDeleteOrder(o.id)} className="p-2 rounded-xl text-zinc-500 hover:text-red-400 hover:bg-red-400/10 border border-transparent hover:border-red-400/20 transition-all cursor-pointer flex items-center justify-center " title={t("common.delete")}>
                                                                            <TrashIcon className="w-5 h-5" />
                                                                        </button>
                                                                        <button onClick={() => onApproveOrder(o.id)} className="flex-1 sm:flex-none max-w-50 px-6 py-2.5 rounded-xl bg-primary hover:bg-primary-hover text-black font-extrabold text-[12px] transition-all cursor-pointer shadow-lg shadow-primary/20 text-center ml-auto">
                                                                            {t("orders.btnApprove")}
                                                                        </button>
                                                                    </>
                                        ...
                                        <div className="space-y-3 animate-fade-in">
                                            {reports.length === 0 ? (
                                                <div className="py-20 flex h-full flex-col items-center justify-center text-zinc-500 bg-white/1 rounded-3xl border border-dashed border-white/5">
                                                    ...
                                                    <p className="font-bold text-sm tracking-wide">{t("reservations.noReports")}</p>
                                                </div>
                                            ) : (
                                                reports.map((r) => (
                                ...
                                <div className="shrink-0 pt-6 mt-4 border-t border-white/5 w-full flex flex-wrap justify-end gap-3">
                                    ...
                                    {reservation.accepted && !reservation.activated && !reservation.completed && (
                                        <>
